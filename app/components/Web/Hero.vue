<template>
  <section class="relative">
    <!-- Hero Section -->
    <div
      class="relative pt-10 min-h-screen flex items-center bg-gradient-to-br from-slate-50 to-emerald-50"
    >
      <!-- Background Pattern -->
      <div class="absolute inset-0 z-0">
        <div
          class="absolute inset-0 bg-[url('/patterns/grid.svg')] opacity-5"
        ></div>
        <!-- Animated gradient blob -->
        <div
          class="absolute top-1/4 right-1/4 w-96 h-96 rounded-full filter blur-3xl animate-blob bg-emerald-500/10"
        ></div>
        <div
          class="absolute bottom-1/4 left-1/4 w-96 h-96 bg-slate-500/10 rounded-full filter blur-3xl animate-blob animation-delay-2000"
        ></div>
      </div>
      ...
      <div class="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div
          class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[calc(100vh-5rem)]"
        >
          <!-- Left Content -->
          <div
            class="space-y-8 transform transition-all duration-700 mx-auto text-center lg:text-left lg:mx-0 max-w-2xl lg:max-w-none"
            :class="{
              'translate-x-0 opacity-100': isContentVisible,
              '-translate-x-10 opacity-0': !isContentVisible,
            }"
          >
            <div class="space-y-4">
              <span
                class="inline-block px-4 py-1 bg-emerald-100 text-emerald-600 rounded-full text-sm font-medium"
              >
              ...
              >
                <span
                  class="block transform transition-all duration-700 delay-100"
                  :class="{
                    'translate-y-0 opacity-100': isContentVisible,
                    'translate-y-10 opacity-0': !isContentVisible,
                  }"
                >
                  {{ slides[currentSlide].title }}
                </span>
                <span
                  class="block transform transition-all duration-700 delay-200 text-emerald-600"
                  :class="{
                    'translate-y-0 opacity-100': isContentVisible,
                    'translate-y-10 opacity-0': !isContentVisible,
                  }"
            ...
            <p
              class="text-xl text-slate-600 max-w-lg mx-auto lg:mx-0 transform transition-all duration-700 delay-300"
              :class="{
                'translate-y-0 opacity-100': isContentVisible,
                'translate-y-10 opacity-0': !isContentVisible,
              }"
            ...
            <div
              class="flex flex-wrap justify-center lg:justify-start gap-4 transform transition-all duration-700 delay-400"
              :class="{
                'translate-y-0 opacity-100': isContentVisible,
                'translate-y-10 opacity-0': !isContentVisible,
              }"
            ...
            <!-- Slide Navigation -->
            <div
              class="flex justify-center lg:justify-start items-center space-x-6 transform transition-all duration-700 delay-500"
              :class="{
                'translate-y-0 opacity-100': isContentVisible,
                'translate-y-10 opacity-0': !isContentVisible,
              }"
          ...
          <div class="relative hidden lg:block">
            ...
            >
              <img
                ...
                :class="{
                  'scale-100': isContentVisible,
                  'scale-95': !isContentVisible,
                }"
              />
              ...
              >
                <div class="flex items-center space-x-2">
                  <span class="flex h-3 w-3">
                    <span
                      class="animate-ping absolute inline-flex h-3 w-3 rounded-full bg-emerald-600 opacity-75"
                    ></span>
                    <span
                      class="relative inline-flex rounded-full h-3 w-3 bg-emerald-600"
                    ></span>
                  </span>
...
const slides = [
  {
    ...
    description:
      "Upgrade your home with our eco-friendly, durable, and stylish plastic products.",
  ...
  },
  {
    ...
    description:
      "Discover high-quality machines and accessories designed to streamline operations with efficiency.",
  ...
  },
  {
    ...
    description:
      "Enhance your outdoor spaces with our eco-friendly garden pots and accessories.",
    buttonText: "Shop Now",
